...
def recordXLSX(data, DR, classifierVariables, precision, recall, f1, testScore):
    kernel = classifierVariables["kernel"]
    data.loc[len(data.index)] = [DR, kernel, precision, recall, f1, testScore] # type: ignore
...
def barBlot(DR, data):
    ...
    names = data[data.DR == DR]["Kernel"].tolist()
...
def MLprocess(dataset, padded_docs, columns, classifierVariables, datasetName, vars, Ctime):
    ...
    data = pd.DataFrame(columns=["DR", "Kernel", "Precision-score", "Recall-score", "F1-score", "Accuracy"])
    ...
    for column in columns:
        ...
        for row in classifierVariables:
            ...
            kernel = classifierVariables[row]["kernel"]
