...
def recordXLSX(data, DR, classifierVariables, precision, recall, f1, testScore):
    weights = classifierVariables["weights"]

    data.loc[len(data.index)] = [DR, weights, precision, recall, f1, testScore] # type: ignore



def barBlot(DR, data):
    variables = ["weights"]
    weights = data[data.DR == DR]["weights"].tolist()

    names = list()
    for index in range(len(weights)):
        names.append(weights[index])
...
def MLprocess(dataset, padded_docs, columns, classifierVariables, datasetName, vars, Ctime):
    ...
    data = pd.DataFrame(columns=["DR", "weights", "Precision-score", "Recall-score", "F1-score", "Accuracy"])
    ...
    for column in columns:
        ...
        for row in classifierVariables:
            ...
            weights = classifierVariables[row]["weights"]
            ...
            trainScore, testScore, labelPredicted = modelEvaluation(knn, train, test)
            classifierVars = {"weights": weights}
...
# fit the model
classifierVariables = {
    "firstRow": {"weights": "uniform"},
    "secondRow": {"weights": "distance"},
    }
variables = ["weights"]
